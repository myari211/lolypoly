...
class WarehouseApprovalController extends Controller
{
    ...
    public function index()
    {
        ...
        $data_transaction = Transaction::with('customer','category','gudang','driver','packaging')
                ->where('row_status','=','1')
                ->get();
        $this->data['data_transaction'] = $data_transaction;

        return view('admin.wh_approval.index', $this->data);
    }
    ...
    public function store(Request $request){
        ...
        try {
            ...
            $data = array(
                ...
                "status_kemasan" => 'CODThjco1277001593ThjcouhG',
                ...
                'status' => 'YAKgiBMT0188001593giBMTRGA'
            );
    ...
    {
        $data = Transaction::with('customer','category','categoryBussines','status_category','user_created','gudang','statusKemasan')
            ->where('row_status','=','1')
            ->whereNotNull('warehouse_check_at')
            ->whereHas('status_category', function($q) {
                $q->where('status_category_code', '=', 'A10');
            })
        ...
        return DataTables::of($data)
            ...
            ->editColumn('lost_kg', function($data) {
                return $data->quantity_kg - $data->real_quantity_kg ;
            })
            ->editColumn('lost_liter', function($data) {
                return $data->quantity_liter - $data->real_quantity_liter ;
            })
            ->editColumn('status_kemasan', function($data) {
                return isset($data->statusKemasan) ? $data->statusKemasan->title : '-';
            })
            ->editColumn('action', function($data) {
                ...
                $url_edit = route('wh_approval.edit',$id_encode);
