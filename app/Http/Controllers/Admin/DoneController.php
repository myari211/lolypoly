...
class DoneController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.transaction.done.index', $this->data);
    }
    ...
    public function edit($id_encode)
    {
        try {        
            ...
            $id = $id->id;
            $data = Transaction::with('detail','statusTransaction','address','store')
                    ->where('row_status','1')
                    ->where('id',$id)
                    ->first();
            $arr_detail = array();
            ...
            if($data->shipping_method_id == 'pckptstr'){
                $data->shipping_detail = 'Pickup At Store ('. $data->store->title .')';
            } else {
    ...
    public function view($id)
    {
        ...
        return view('admin.transaction.done.view', $this->data);
    }

    public function getAll(Request $request)
    {
        $data = Transaction::with('detail','statusTransaction','address')
        ->whereIn('status', array("7"))
        ...
        ->editColumn('order_type', function($data) {
            return $data->shipping_method_id == 'pckptstr' ? 'Pickup At Store' : ucfirst($data->shipping_method_id);
        })
        ...
        ->editColumn('action', function($data) {
            ...
            $url_view = route('transaction.done.view',$data->id);
            $url_edit = route('transaction.done.edit',$id_encode);
            $url_delete = route('transaction.done.destroy',$id_encode);
