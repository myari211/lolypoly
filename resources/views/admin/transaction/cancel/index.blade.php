@extends('layouts.admin.app', ['title' => 'cancel','parent' => 'Transaction'])
...
@section('content')
	...
	<div class="modal fade" id="modal-process" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="modal-processLabel" aria-hidden="true">
		...
		<div class="modal-header">
			<h5 class="modal-title" id="modalAddLabel">Cancel</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			<span aria-hidden="true">&times;</span>
			</button>
		</div>
		<form action="{{route('transaction.cancel.store')}}" id="submit-form">
...
@push('body-scripts')
	<script>
		$("#table-all").DataTable({
			...
			"autoWidth": true,
			"ajax": {
				"url": "{{ route('transaction.cancel.getAll') }}",
				"type": "GET",
				"datatype": "JSON",
