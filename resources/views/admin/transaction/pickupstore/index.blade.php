@extends('layouts.admin.app', ['title' => 'pickupstore','parent' => 'Transaction'])

@section('title', 'Pickup At Store')
...
@section('content')
	...
	<div class="modal fade" id="modal-process" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="modal-processLabel" aria-hidden="true">
		...
		<form action="{{route('transaction.pickupstore.store')}}" id="submit-form">
...
@push('body-scripts')
	<script>
		$("#table-all").DataTable({
			...
			"autoWidth": true,
			"ajax": {
				"url": "{{ route('transaction.pickupstore.getAll') }}",
				"type": "GET",
				"datatype": "JSON",
			...
			"columns": [
				...
				{ data: "store_name", name: "store_name", className:"clickable-order"},
